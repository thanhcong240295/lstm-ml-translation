...
class Seq2SeqLanguageTranslation:
    def __init__(
        ...
        if embedding_matrix_src is not None:
            ...
            self.embedding_src = self.xp.random.randn(len(vocab_src.word2idx), input_size).astype(self.xp.float32) * 0.01

        if embedding_matrix_tgt is not None:
            self.embedding_tgt = self.xp.asarray(embedding_matrix_tgt, dtype=self.xp.float32)
        else:
            self.embedding_tgt = self.xp.random.randn(len(vocab_tgt.word2idx), input_size).astype(self.xp.float32) * 0.01
    ...
    def save_model(self, path):
        ...
        def save_lstm(lstm):
            return {
                "Wf": to_cpu(lstm.Wf), "Uf": to_cpu(lstm.Uf), "bf": to_cpu(lstm.bf),
                "Wi": to_cpu(lstm.Wi), "Ui": to_cpu(lstm.Ui), "bi": to_cpu(lstm.bi),
                "Wo": to_cpu(lstm.Wo), "Uo": to_cpu(lstm.Uo), "bo": to_cpu(lstm.bo),
                "Wc": to_cpu(lstm.Wc), "Uc": to_cpu(lstm.Uc), "bc": to_cpu(lstm.bc),
